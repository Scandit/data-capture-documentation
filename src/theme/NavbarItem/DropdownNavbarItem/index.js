...
function DropdownNavbarItemDesktop({
  ...
  const xamarinVersion = useMemo(() => {
    if (!currentPath) return "/7.6.13";
    if (currentPath.includes("/7.6.13")) return "/7.6.13";
    if (currentPath.includes("/6.28.9")) return "/6.28.9";
    if (currentPath.includes("/7.6.")) return "/7.6.13";
    if (currentPath.includes("/6.28.")) return "/6.28.9";
    return "/7.6.13"; // Default to 7.6.13 for Xamarin
  }, [currentPath]);

  useEffect(() => {
    if (!currentPath) return;
    const possibleVersions = ["/next", "/6.28.9", "/7.6.13"];
