...
These capabilities work together and are **enabled by default** in SDK 8.1+. In most cases, no additional configuration is required.
...
<ReactPlayer controls width='1200' url="/img/ai-powered-scanning/intention.mp4" />
...
<ReactPlayer controls width='1200' url="/img/ai-powered-scanning/dense-selection.mp4" />
...
However, when the barcode's encoded information is physically lost or damaged beyond reconstruction, such as when critical sections are torn away, burned, or chemically degraded, our text recognition technology helps to get the data. 
...
<ReactPlayer controls width='1200' url="/img/ai-powered-scanning/ocr-fallback.mp4" />
...
1. **Workflow delays** – When intentionally scanning duplicate items (for example, multiple boxes or items with the same barcode during inventory), users must wait for the timeout to expire between each scan. This artificial delay slows down operations.
...
<ReactPlayer controls width='1200' url="/img/ai-powered-scanning/duplicate-filter.mp4" />