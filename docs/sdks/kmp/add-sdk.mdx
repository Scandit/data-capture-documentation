---
description: "Integrate the Scandit Data Capture SDK into a Kotlin Multiplatform project, on both Android and iOS."
...
| `parser` | Structured data parsing for supported formats (for example, GS1, driver's license barcodes). |
...
In Kotlin (shared code) the same call is `DataCaptureContext.initialize(licenseKey)`—initializing the context from shared Kotlin code (as the sample apps do) keeps the Swift side free of these bridging details and is the recommended setup.
