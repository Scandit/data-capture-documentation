...
Import the product you added by its product name. The Kotlin API surfaces under the plain Scandit type names (`DataCaptureContext`, `BarcodeCapture`, …); Kotlin `companion object` factories are reachable through `.companion` in Swift:

```swift
import ScanditKmpBarcode

let context = DataCaptureContext.companion.initialize(licenseKey: "-- ENTER YOUR SCANDIT LICENSE KEY HERE --")
```
