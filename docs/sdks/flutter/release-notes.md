...
* Added `BarcodeArPopoverAnnotation.Anchor` and `BarcodeArPopoverAnnotationAnchor` enum to the Flutter binding.
* Added the ability to filter barcodes in `BarcodeArView` through the `BarcodeArFilter` interface.
