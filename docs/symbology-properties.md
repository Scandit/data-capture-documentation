...
| ocr_fallback | Enables Optical Character Recognition of text as a fallback when other readers fail. This feature requires Smart Scan Intention which is only available in SparkScan or through the Linux settings preset `SC_PRESET_SINGLE_CODE_HAND_HELD`.<br /><br />Optionally set a RegEx pattern to constrain results using `SymbologySettings.ocrFallbackRegex()`.<br /><br />**Requires the `LabelCaptureText` module.** |
